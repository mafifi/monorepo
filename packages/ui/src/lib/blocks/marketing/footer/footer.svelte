<script lang="ts">
    ...
    let { 
        ...
        logoSvg = "",
        brandName = "Flowbite",
        description = "Flowbite is a open-source library of over 400+ web components and interactive elements built with the utility classes from Tailwind CSS.",
        socialLinks = [
            {
                name: "Facebook",
                url: "#",
    ...
    <div class="p-4 py-6 mx-auto max-w-screen-xl md:p-8 lg:p-10">
        <div class="grid grid-cols-2 gap-8 lg:grid-cols-6">
            <div class="col-span-2">
                <a href={logoUrl} class="flex items-center mb-2 text-2xl font-semibold text-zinc-900 sm:mb-0 dark:text-white">
                    {#if logoSvg}
                        {@html logoSvg}
                    {:else}
                ...
                <p class="my-4 font-light text-zinc-500 dark:text-zinc-400">{description}</p>
                <ul class="flex mt-5 space-x-6">
                    {#each socialLinks as social}
                        <li>
                            <a href={social.url} class="text-zinc-500 hover:text-zinc-900 dark:hover:text-white dark:text-zinc-400" aria-label={social.name}>
                                {@html social.icon}
                            </a>
                        </li>
                    {/each}
            ...
            {#each columns as column}
                <div class="lg:mx-auto">
                    <h2 class="mb-6 text-sm font-semibold text-zinc-900 uppercase dark:text-white">{column.title}</h2>
        ...
        <span class="block text-sm text-center text-zinc-500 dark:text-zinc-400">© {yearText} <a href={copyrightLink} class="hover:underline">{copyrightName}</a>. {copyrightText}</span>
    </div>
</footer>